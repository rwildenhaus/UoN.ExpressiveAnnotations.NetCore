...
var
    ...
    annotations = [],
    annotationLimit = 100,
    annotationChars = ' abcdefghijklmnopqrstuvwxyz'; // suffixes for attributes annotating single field multiple times

    for (var i = 0; i < annotationLimit; i++) { 
        var suffixCharA = annotationChars[i / annotationChars.length];
        var suffixCharB = annotationChars[i % annotationChars.length];

        var suffix = suffixCharA + suffixCharB;
        annotations.push(suffix);
    }

    // bind requirements first...
    $.each(annotations, function() { // it would be ideal to have exactly as many handlers as there are unique annotations, but the number of annotations isn't known untill DOM is ready
